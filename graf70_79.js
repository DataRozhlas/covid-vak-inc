(function () {
  const population = 1038378; // lidi 70 - 79 dle ČSÚ
  Promise.all([
    ...
    infected.forEach((rec, i) => {
      if (i >= 21) {
        ...
        let suma = 0;
        threeWeeks.forEach((r) => { suma += r['70-74'] + r['75-79']; });
        inf_srs.push([
          Date.parse(rec.ind),
    ...
    // vakcinovaní 70 - 79 celkem
    const vacc_srs = [];
    let vsum = 0;
    vaccinated.forEach((rec) => {
      vsum += rec['70-74'] + rec['75-79'];
      vacc_srs.push([
        Date.parse(rec.ind),
    ...
    Highcharts.chart('cvd-vak-inc-70_79', {
      chart: {
        type: 'line',
        spacingLeft: 0,
        spacingRight: 0,
      ...
      title: {
        text: 'Očkovaní a nakažení ve věku 70 - 79',
        align: 'left',
        style: {
          fontWeight: 'bold',
        },
